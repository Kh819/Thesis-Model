...
to setup
  ...
  set carbon-price-per-ton 21.03
  set carbon-variance initial-vol
  set rho -0.8
  set kappa 12
  set theta 0.000361

  let size-class-list shuffle (sentence
 (n-values 167 [i -> "low"])
 (n-values 167 [i -> "mid"])
 (n-values 166 [i -> "high"])
)
...
to evaluate-bio-adoption
  ask turtles [
    ...
    if available-for-conversion <= 0.01 [ stop ]
  ...
  set peer-adjustment exp (peer-effect-weight * peer-rate)
    ...
    if adjusted-bio-utility > conv-utility [
      let adoption-amount min (list 0.01 available-for-conversion)
...
<experiments>
  ...
  <experiment name="vol-of-vol 90" repetitions="25" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks &gt;= 26</exitCondition>
    <metric>mean [item 2 crop-allocation] of turtles</metric>
    <enumeratedValueSet variable="discount-rate">
      <value value="5"/>
    </enumeratedValueSet>
    <steppedValueSet variable="vol-of-vol" first="0.2" step="0.5" last="0.8"/>
    <enumeratedValueSet variable="risk-averse">
      <value value="-1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="peer-effect-weight">
